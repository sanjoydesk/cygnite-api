       ...
       public  function urlstucture($defaultController="") 
        {          
                    ...
                    if(file_exists(APPPATH."controllers".DS.$uristring[$indexCount+1].EXT)) {
                                  ...
                                  $controller_name = $this->buildControllerName($uristring[$indexCount+1]);
                                 $_obj_controller = new $controller_name();
                     }                     
                     if(empty($indexCount) AND empty($_obj_controller)) {     
                                require_once(APPPATH."controllers".DS.$defaultController.EXT);
                            
                                $controller_name = $this->buildControllerName($defaultController);
             ...
             private function buildControllerName($controller_name)
            {                 
                 if(!empty($controller_name))
                        $controller_standard_name = ucfirst($controller_name).ucfirst(str_replace('/', '',APPPATH)).'Controller';
                 return $controller_standard_name;
            }
            
           public function site_url($uri = '') {
                    //$base_url = $this->get_base_url().DS.$uri;
                    return $uri;
           }
